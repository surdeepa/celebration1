...
// Removed Recharts imports: BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer
...
const AdminDashboard: React.FC = () => {
  ...
  const handleDeleteStaff = (id: string) => {
    if (confirm("Delete this staff member?")) {
      syncStaff(staff.filter(s => s.id !== id));
    }
  };
  ...
  const handleAddCustomer = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    syncCustomers([...customers, newCustomer]);
    e.currentTarget.reset();
  };
  ...
  // Removed statsData computation as the chart is no longer displayed
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'alerts' && (
        <div className="bg-white rounded-[32px] shadow-sm border border-slate-100 overflow-hidden">
          <div className="p-8 border-b bg-red-50/30">
            <h3 className="text-xl font-black text-red-900">Missed Action Alerts</h3>
            <p className="text-red-600 font-medium text-sm">Review staff performance on overdue tasks.</p>
          </div>
              ...
              <tbody className="divide-y divide-slate-50">
                {alerts.map((alert, i) => (
                  <tr key={i} className="hover:bg-red-50/20 transition-colors">
                    ...
                    <td className="px-8 py-6 font-bold text-red-600 text-sm whitespace-nowrap">{alert.daysLate} Days Overdue</td>
                  </tr>
                ))}
                {alerts.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-8 py-12 text-center text-slate-400 font-medium italic">All staff members are on schedule.</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      {activeTab === 'staff' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 h-fit">
            ...
            <form key={editingStaff?.id || 'new-staff-form'} onSubmit={editingStaff ? handleUpdateStaff : handleAddStaff} className="space-y-4">
              ...
              <button type="submit" className="w-full bg-indigo-600 text-white font-black py-4 rounded-2xl shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all">
      ...
      {activeTab === 'customers' && (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 h-fit">
            <h3 className="text-xl font-bold mb-6">{editingCustomer ? 'Edit Customer' : 'Feed New Record'}</h3>
            <form key={editingCustomer?.id || 'new-customer-form'} onSubmit={editingCustomer ? handleUpdateCustomer : handleAddCustomer} className="space-y-4">
              <Input name="name" def={editingCustomer?.name} placeholder="Customer Full Name" />
              <Input name="phone" def={editingCustomer?.phone} placeholder="Phone (e.g. +91 ...)" />
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Assign to Staff Member</label>
                <select required name="staffId" defaultValue={editingCustomer?.assignedStaffId || ''} className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 font-black text-slate-700">
              ...
              <button type="submit" className="w-full bg-indigo-600 text-white font-black py-4 rounded-2xl shadow-lg hover:bg-indigo-700 transition-all">
                {editingCustomer ? 'Save Changes' : 'Feed Record'}
              </button>
              {editingCustomer && (
          ...
          <div className="lg:col-span-3 space-y-4">
            <div className="bg-slate-900 text-white p-5 rounded-[24px] flex flex-wrap gap-8 items-center justify-center text-[11px] font-black uppercase tracking-widest shadow-xl border-t-4 border-indigo-500">
              ...
              <div className="flex items-center gap-2"><div className="w-2 h-2 bg-indigo-500 rounded-full"></div> C: Tele Call (3d Before)</div>
              <div className="flex items-center gap-2"><div className="w-2 h-2 bg-indigo-500 rounded-full"></div> G: Greet (On Big Day)</div>
              <div className="flex items-center gap-2"><div className="w-2 h-2 bg-indigo-500 rounded-full"></div> F: Follow-up (2d After)</div>
            </div>
            <div className="bg-white rounded-[32px] shadow-sm border border-slate-100 overflow-hidden overflow-x-auto">
              <table className="w-full text-left min-w-[850px]">
                <thead className="bg-slate-50 border-b">
                  <tr>
                    <th className="px-8 py-5 text-[10px] font-bold text-slate-400 uppercase">Customer Information</th>
                    <th className="px-8 py-5 text-[10px] font-bold text-slate-400 uppercase text-center">Date</th>
                    <th className="px-8 py-5 text-[10px] font-bold text-slate-400 uppercase">Assigned Staff</th>
                    <th className="px-8 py-5 text-[10px] font-bold text-slate-400 uppercase text-center">Compliance Stages</th>
                    <th className="px-8 py-5 text-[10px] font-bold text-slate-400 uppercase text-right">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {customers.map(c => (
                    <tr key={c.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-8 py-6">
                        ...
                        <p className="text-[10px] text-slate-400 font-black uppercase tracking-tighter">{c.eventType} • {c.phone}</p>
                      </td>
                      <td className="px-8 py-6 text-sm font-black text-indigo-600 text-center whitespace-nowrap">{c.day} {MONTHS[c.month]}</td>
...
const Input = ({ name, placeholder, def, type = "text" }: any) => (
  <input required name={name} defaultValue={def} type={type} placeholder={placeholder} className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 font-medium placeholder:text-slate-300" />
);
const Dot = ({ active, label }: any) => (
  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-[10px] font-black transition-all ${active ? 'bg-indigo-600 text-white shadow-lg scale-110' : 'bg-slate-100 text-slate-300 border border-slate-200 opacity-40'}`}>{label}</div>
);
